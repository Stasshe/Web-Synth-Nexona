...
export class EQ {
  private lowL = new BiquadFilter(); private lowR = new BiquadFilter();
  private midL = new BiquadFilter(); private midR = new BiquadFilter();
  private highL = new BiquadFilter(); private highR = new BiquadFilter();
  private lowGain = 0; private midGain = 0; private highGain = 0;
  private mix = 0;
  ...
  setParams(lowGain: number, midGain: number, highGain: number, mix: number): void {
    ...
    if (lowGain !== this.lowGain || midGain !== this.midGain || highGain !== this.highGain) {
      this.lowGain = lowGain; this.midGain = midGain; this.highGain = highGain;
      this.lowL.setLowShelf(250, lowGain, sr);  this.lowR.setLowShelf(250, lowGain, sr);
      this.midL.setPeaking(1000, midGain, 1, sr); this.midR.setPeaking(1000, midGain, 1, sr);
      this.highL.setHighShelf(4000, highGain, sr); this.highR.setHighShelf(4000, highGain, sr);
    }
    this.mix = mix;
  }
