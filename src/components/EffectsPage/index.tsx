...
// --- Individual effect controls (horizontal knob rows) ---
...
function DistortionControls() {
  ...
  return (
    <>
      ...
      <Knob
        label="Drive" value={d.drive} min={1} max={100} step={0.1}
        onChange={(v) => (synthState.effects.distortion.drive = v)}
        color="var(--effects)" formatValue={(v) => `${v.toFixed(1)}`} size={28}
        modRoutes={modDrive} onModDrop={handleModDrop(ModTarget.DIST_DRIVE)} modTarget={ModTarget.DIST_DRIVE}
      />
      <Knob
        label="Tone" value={d.tone} min={0} max={1}
        onChange={(v) => (synthState.effects.distortion.tone = v)}
        color="var(--effects)" size={28}
      />
      <Knob
        label="Mix" value={d.mix} min={0} max={1}
        onChange={(v) => (synthState.effects.distortion.mix = v)}
        color="var(--effects)" size={28}
        modRoutes={modMix} onModDrop={handleModDrop(ModTarget.DIST_MIX)} modTarget={ModTarget.DIST_MIX}
      />
    </>
  );
}
...
function CompressorControls() {
  ...
  return (
    <>
      <Knob
        label="Thresh" value={c.threshold} min={-60} max={0} step={0.5}
        onChange={(v) => (synthState.effects.compressor.threshold = v)}
        color="var(--effects)" formatValue={(v) => `${v.toFixed(0)}dB`} size={28}
      />
      <Knob
        label="Ratio" value={c.ratio} min={1} max={20} step={0.5}
        onChange={(v) => (synthState.effects.compressor.ratio = v)}
        color="var(--effects)" formatValue={(v) => `${v.toFixed(1)}:1`} size={28}
      />
      <Knob
        label="Atk" value={c.attack} min={0.001} max={0.5} step={0.001}
        onChange={(v) => (synthState.effects.compressor.attack = v)}
        color="var(--effects)" formatValue={(v) => `${(v * 1000).toFixed(0)}ms`} size={28}
      />
      <Knob
        label="Rel" value={c.release} min={0.01} max={2} step={0.01}
        onChange={(v) => (synthState.effects.compressor.release = v)}
        color="var(--effects)" formatValue={(v) => `${(v * 1000).toFixed(0)}ms`} size={28}
      />
      <Knob
        label="Gain" value={c.makeup} min={0} max={36} step={0.5}
        onChange={(v) => (synthState.effects.compressor.makeup = v)}
        color="var(--effects)" formatValue={(v) => `+${v.toFixed(0)}dB`} size={28}
      />
      <Knob
        label="Mix" value={c.mix} min={0} max={1}
...
function ChorusControls() {
  ...
  return (
    <>
      <Knob
        label="Rate" value={c.rate} min={0.1} max={5} step={0.01}
        onChange={(v) => (synthState.effects.chorus.rate = v)}
        color="var(--effects)" formatValue={(v) => `${v.toFixed(1)}Hz`} size={28}
        modRoutes={modRate} onModDrop={handleModDrop(ModTarget.CHORUS_RATE)} modTarget={ModTarget.CHORUS_RATE}
      />
      <Knob
        label="Depth" value={c.depth} min={0} max={1}
        onChange={(v) => (synthState.effects.chorus.depth = v)}
        color="var(--effects)" size={28}
        modRoutes={modDepth} onModDrop={handleModDrop(ModTarget.CHORUS_DEPTH)} modTarget={ModTarget.CHORUS_DEPTH}
      />
      <Knob
        label="Mix" value={c.mix} min={0} max={1}
        onChange={(v) => (synthState.effects.chorus.mix = v)}
        color="var(--effects)" size={28}
        modRoutes={modMix} onModDrop={handleModDrop(ModTarget.CHORUS_MIX)} modTarget={ModTarget.CHORUS_MIX}
      />
    </>
  );
}
...
function FlangerControls() {
  ...
  return (
    <>
      <Knob
        label="Rate" value={f.rate} min={0.01} max={10} step={0.01}
        onChange={(v) => (synthState.effects.flanger.rate = v)}
        color="var(--effects)" formatValue={(v) => `${v.toFixed(2)}Hz`} size={28}
        modRoutes={modRate} onModDrop={handleModDrop(ModTarget.FLANGER_RATE)} modTarget={ModTarget.FLANGER_RATE}
      />
      <Knob
        label="Depth" value={f.depth} min={0} max={1}
        onChange={(v) => (synthState.effects.flanger.depth = v)}
        color="var(--effects)" size={28}
        modRoutes={modDepth} onModDrop={handleModDrop(ModTarget.FLANGER_DEPTH)} modTarget={ModTarget.FLANGER_DEPTH}
      />
      <Knob
        label="Fdbk" value={f.feedback} min={0} max={0.95}
        onChange={(v) => (synthState.effects.flanger.feedback = v)}
        color="var(--effects)" size={28}
        modRoutes={modFeedback} onModDrop={handleModDrop(ModTarget.FLANGER_FEEDBACK)} modTarget={ModTarget.FLANGER_FEEDBACK}
      />
      <Knob
        label="Mix" value={f.mix} min={0} max={1}
        onChange={(v) => (synthState.effects.flanger.mix = v)}
        color="var(--effects)" size={28}
        modRoutes={modMix} onModDrop={handleModDrop(ModTarget.FLANGER_MIX)} modTarget={ModTarget.FLANGER_MIX}
      />
    </>
  );
}
...
function PhaserControls() {
  ...
  return (
    <>
      <Knob
        label="Rate" value={p.rate} min={0.01} max={10} step={0.01}
        onChange={(v) => (synthState.effects.phaser.rate = v)}
        color="var(--effects)" formatValue={(v) => `${v.toFixed(2)}Hz`} size={28}
        modRoutes={modRate} onModDrop={handleModDrop(ModTarget.PHASER_RATE)} modTarget={ModTarget.PHASER_RATE}
      />
      <Knob
        label="Depth" value={p.depth} min={0} max={1}
        onChange={(v) => (synthState.effects.phaser.depth = v)}
        color="var(--effects)" size={28}
        modRoutes={modDepth} onModDrop={handleModDrop(ModTarget.PHASER_DEPTH)} modTarget={ModTarget.PHASER_DEPTH}
      />
      <Knob
        label="Fdbk" value={p.feedback} min={0} max={0.95}
        onChange={(v) => (synthState.effects.phaser.feedback = v)}
        color="var(--effects)" size={28}
        modRoutes={modFeedback} onModDrop={handleModDrop(ModTarget.PHASER_FEEDBACK)} modTarget={ModTarget.PHASER_FEEDBACK}
      />
      <Knob
        label="Mix" value={p.mix} min={0} max={1}
        onChange={(v) => (synthState.effects.phaser.mix = v)}
        color="var(--effects)" size={28}
        modRoutes={modMix} onModDrop={handleModDrop(ModTarget.PHASER_MIX)} modTarget={ModTarget.PHASER_MIX}
      />
    </>
  );
}
...
function DelayControls() {
  ...
  return (
    <>
      <Knob
        label="Time" value={d.time} min={0.01} max={2} step={0.01}
        onChange={(v) => (synthState.effects.delay.time = v)}
        color="var(--effects)" formatValue={(v) => `${(v * 1000).toFixed(0)}ms`} size={28}
        modRoutes={modTime} onModDrop={handleModDrop(ModTarget.DELAY_TIME)} modTarget={ModTarget.DELAY_TIME}
      />
      <Knob
        label="Fdbk" value={d.feedback} min={0} max={0.95}
        onChange={(v) => (synthState.effects.delay.feedback = v)}
        color="var(--effects)" size={28}
        modRoutes={modFeedback} onModDrop={handleModDrop(ModTarget.DELAY_FEEDBACK)} modTarget={ModTarget.DELAY_FEEDBACK}
      />
      <Knob
        label="Mix" value={d.mix} min={0} max={1}
        onChange={(v) => (synthState.effects.delay.mix = v)}
        color="var(--effects)" size={28}
        modRoutes={modMix} onModDrop={handleModDrop(ModTarget.DELAY_MIX)} modTarget={ModTarget.DELAY_MIX}
      />
    </>
  );
}
...
function ReverbControls() {
  ...
  return (
    <>
      <Knob
        label="Decay" value={r.decay} min={0.1} max={0.99}
        onChange={(v) => (synthState.effects.reverb.decay = v)}
        color="var(--effects)" size={28}
        modRoutes={modDecay} onModDrop={handleModDrop(ModTarget.REVERB_DECAY)} modTarget={ModTarget.REVERB_DECAY}
      />
      <Knob
        label="Mix" value={r.mix} min={0} max={1}
        onChange={(v) => (synthState.effects.reverb.mix = v)}
        color="var(--effects)" size={28}
        modRoutes={modMix} onModDrop={handleModDrop(ModTarget.REVERB_MIX)} modTarget={ModTarget.REVERB_MIX}
      />
    </>
  );
}
...
function EQControls() {
  ...
  return (
    <>
      <Knob
        label="Low" value={eq.lowGain} min={-12} max={12} step={0.5}
        onChange={(v) => (synthState.effects.eq.lowGain = v)}
        color="var(--effects)" formatValue={(v) => `${v > 0 ? "+" : ""}${v.toFixed(0)}dB`} size={28}
        modRoutes={modLow} onModDrop={handleModDrop(ModTarget.EQ_LOW)} modTarget={ModTarget.EQ_LOW}
      />
      <Knob
        label="Mid" value={eq.midGain} min={-12} max={12} step={0.5}
        onChange={(v) => (synthState.effects.eq.midGain = v)}
        color="var(--effects)" formatValue={(v) => `${v > 0 ? "+" : ""}${v.toFixed(0)}dB`} size={28}
        modRoutes={modMid} onModDrop={handleModDrop(ModTarget.EQ_MID)} modTarget={ModTarget.EQ_MID}
      />
      <Knob
        label="High" value={eq.highGain} min={-12} max={12} step={0.5}
        onChange={(v) => (synthState.effects.eq.highGain = v)}
        color="var(--effects)" formatValue={(v) => `${v > 0 ? "+" : ""}${v.toFixed(0)}dB`} size={28}
        modRoutes={modHigh} onModDrop={handleModDrop(ModTarget.EQ_HIGH)} modTarget={ModTarget.EQ_HIGH}
      />
      <Knob
        label="Mix" value={eq.mix} min={0} max={1}
