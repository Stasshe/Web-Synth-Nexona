  ...
  return (
    <Panel title="DISTORTION" color="var(--effects)"
      ...
      <div className="grid grid-cols-3 gap-1">
        <Knob label="Drive" value={d.drive} min={1} max={20} step={0.1}
          onChange={(v) => (synthState.effects.distortion.drive = v)}
          color="var(--effects)" formatValue={(v) => `${v.toFixed(1)}x`} size={32} />
        <Knob label="Tone" value={d.tone} min={0} max={1}
          onChange={(v) => (synthState.effects.distortion.tone = v)}
          color="var(--effects)" size={32} />
        <Knob label="Mix" value={d.mix} min={0} max={1}
    ...
    <Panel title="COMPRESSOR" color="var(--effects)"
      ...
      <div className="grid grid-cols-3 gap-1">
        <Knob label="Thresh" value={c.threshold} min={-60} max={0} step={0.5}
          ...
          color="var(--effects)" formatValue={(v) => `${v.toFixed(0)}dB`} size={32} />
        <Knob label="Ratio" value={c.ratio} min={1} max={20} step={0.5}
          onChange={(v) => (synthState.effects.compressor.ratio = v)}
          color="var(--effects)" formatValue={(v) => `${v.toFixed(1)}:1`} size={32} />
        <Knob label="Makeup" value={c.makeup} min={0} max={24} step={0.5}
          onChange={(v) => (synthState.effects.compressor.makeup = v)}
          color="var(--effects)" formatValue={(v) => `+${v.toFixed(0)}dB`} size={32} />
        <Knob label="Atk" value={c.attack} min={0.001} max={0.5} step={0.001}
          onChange={(v) => (synthState.effects.compressor.attack = v)}
          color="var(--effects)" formatValue={(v) => `${(v * 1000).toFixed(0)}ms`} size={32} />
        <Knob label="Rel" value={c.release} min={0.01} max={2} step={0.01}
          onChange={(v) => (synthState.effects.compressor.release = v)}
          color="var(--effects)" formatValue={(v) => `${(v * 1000).toFixed(0)}ms`} size={32} />
        <Knob label="Mix" value={c.mix} min={0} max={1}
    ...
    <Panel title="CHORUS" color="var(--effects)"
      ...
      <div className="grid grid-cols-3 gap-1">
        <Knob label="Rate" value={c.rate} min={0.1} max={5} step={0.01}
          ...
          color="var(--effects)" formatValue={(v) => `${v.toFixed(1)}Hz`} size={32} />
        <Knob label="Depth" value={c.depth} min={0} max={1}
    ...
    <Panel title="FLANGER" color="var(--effects)"
      ...
      <div className="grid grid-cols-2 gap-1">
        <Knob label="Rate" value={f.rate} min={0.01} max={10} step={0.01}
          ...
          color="var(--effects)" formatValue={(v) => `${v.toFixed(2)}Hz`} size={32} />
        <Knob label="Depth" value={f.depth} min={0} max={1}
          onChange={(v) => (synthState.effects.flanger.depth = v)}
          color="var(--effects)" size={32} />
        <Knob label="Fdbk" value={f.feedback} min={0} max={0.95}
          onChange={(v) => (synthState.effects.flanger.feedback = v)}
          color="var(--effects)" size={32} />
        <Knob label="Mix" value={f.mix} min={0} max={1}
    ...
    <Panel title="PHASER" color="var(--effects)"
      ...
      <div className="grid grid-cols-2 gap-1">
        <Knob label="Rate" value={p.rate} min={0.01} max={10} step={0.01}
          ...
          color="var(--effects)" formatValue={(v) => `${v.toFixed(2)}Hz`} size={32} />
        <Knob label="Depth" value={p.depth} min={0} max={1}
          onChange={(v) => (synthState.effects.phaser.depth = v)}
          color="var(--effects)" size={32} />
        <Knob label="Fdbk" value={p.feedback} min={0} max={0.95}
          onChange={(v) => (synthState.effects.phaser.feedback = v)}
          color="var(--effects)" size={32} />
        <Knob label="Mix" value={p.mix} min={0} max={1}
    ...
    <Panel title="DELAY" color="var(--effects)"
      ...
      <div className="grid grid-cols-3 gap-1">
        <Knob label="Time" value={d.time} min={0.01} max={2} step={0.01}
          ...
          color="var(--effects)" formatValue={(v) => `${(v * 1000).toFixed(0)}ms`} size={32} />
        <Knob label="Fdbk" value={d.feedback} min={0} max={0.95}
    ...
    <Panel title="REVERB" color="var(--effects)"
      ...
      <div className="grid grid-cols-2 gap-1">
        <Knob label="Decay" value={r.decay} min={0.1} max={0.99}
          ...
          color="var(--effects)" size={32} />
        <Knob label="Mix" value={r.mix} min={0} max={1}
    ...
    <Panel title="EQ" color="var(--effects)"
      ...
      <div className="grid grid-cols-4 gap-1">
        <Knob label="Low" value={eq.lowGain} min={-12} max={12} step={0.5}
          ...
          color="var(--effects)" formatValue={(v) => `${v > 0 ? "+" : ""}${v.toFixed(0)}dB`} size={32} />
        <Knob label="Mid" value={eq.midGain} min={-12} max={12} step={0.5}
          onChange={(v) => (synthState.effects.eq.midGain = v)}
          color="var(--effects)" formatValue={(v) => `${v > 0 ? "+" : ""}${v.toFixed(0)}dB`} size={32} />
        <Knob label="High" value={eq.highGain} min={-12} max={12} step={0.5}
          onChange={(v) => (synthState.effects.eq.highGain = v)}
          color="var(--effects)" formatValue={(v) => `${v > 0 ? "+" : ""}${v.toFixed(0)}dB`} size={32} />
        <Knob label="Mix" value={eq.mix} min={0} max={1}
